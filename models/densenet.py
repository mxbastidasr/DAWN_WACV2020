...
class TransitionBlock(nn.Module):
    def __init__(self, in_planes, out_planes, dropRate=0.0):
        ...
        self.conv1 = nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=1,
                               padding=0, bias=False)
        self.droprate = dropRate
        self.avgpool = nn.AvgPool2d(kernel_size=2, stride=2)#nn.AdaptiveAvgPool2d((1, 1))
    def forward(self, x):
        out = self.conv1(self.relu(self.bn1(x)))
        if self.droprate > 0:
            out = F.dropout(out, p=self.droprate,
                            inplace=False, training=self.training)
        return self.avgpool(out)#F.avg_pool2d(out, 2)
...
class InitialConv(nn.Module):
    def __init__(self, in_planes, out_planes, dropRate=0.0):
        ...
        #odel:add(nn.SpatialMaxPooling(3, 3, 2, 2, 1, 1))
    ...
    def forward(self, x):
        out = self.pooling1(self.relu(self.bn1(self.conv1(x))))
        return out


class DenseNet3(nn.Module):
    def __init__(self, depth, num_classes, growth_rate=12,
                 reduction=0.5, bottleneck=True, dropRate=0.0, init_conv=True):
        ...
        # 1st conv(s) before any dense block
        if(init_conv):
            # For bigger inputs (224x224)
            # conv -> bn -> relu -> maxpool
            # is done to reduce the image size
            self.conv1 = InitialConv(3, in_planes)
        else:
            # For small inputs (32x32), only a simple conv is made
